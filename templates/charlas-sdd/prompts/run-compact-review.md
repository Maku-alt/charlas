...
Antes de revisar, confirma `Workflow mode: compact`, que `worker_id` y `session_id` son distintos de los de build y que el SHA256 del candidato exacto coincide con el handoff. Revisa con `review-charlas`, genera un veredicto independiente y un reporte accionable si requiere cambios.
